...
GContext2D.prototype.drawImage = function(image, // image
    ...
    if (GBridge.isIOS()) {
        //ios 处理
        GBridge.preLoadImage([image, imageId], function(data) {
            //ios处理
            ...
            imageInfo.width = typeof data.width === 'number' ? data.width : 0;
            imageInfo.height = typeof data.height === 'number' ? data.height : 0;


            if (numArgs === 3) {
                var x = parseFloat(params.sx) || 0.0;
                var y = parseFloat(params.sy) || 0.0;

                that._drawCommands += ("d" + imageInfo.id + ",0,0," +
                    imageInfo.width + "," + imageInfo.height + "," +
                    x + "," + y + "," + imageInfo.width + "," + imageInfo.height + ";");
            } else if (numArgs === 5) {
                var x = parseFloat(params.sx) || 0.0;
                var y = parseFloat(params.sy) || 0.0;
                var width = parseInt(params.sw) || imageInfo.width;
                var height = parseInt(params.sh) || imageInfo.height;

                that._drawCommands += ("d" + imageInfo.id + ",0,0," +
                    imageInfo.width + "," + imageInfo.height + "," +
                    x + "," + y + "," + width + "," + height + ";");
            } else if (numArgs === 9) {
                var sx = parseFloat(params.sx) || 0.0;
                var sy = parseFloat(params.sy) || 0.0;
                var sw = parseInt(params.sw) || imageInfo.width;
                var sh = parseInt(params.sh) || imageInfo.height;
                var dx = parseFloat(params.dx) || 0.0;
                var dy = parseFloat(params.dy) || 0.0;
                var dw = parseInt(params.dw) || imageInfo.width;
                var dh = parseInt(params.dh) || imageInfo.height;

                that._drawCommands += ("d" + imageInfo.id + "," +
                    sx + "," + sy + "," + sw + "," + sh + "," +
                    dx + "," + dy + "," + dw + "," + dh + ";");
            }
        ...
        GBridge.preLoadImage(image,function(data){
            if (typeof data === 'string') {
                try {
                    data = JSON.parse(data);
                } catch (err) {}
            ...
            if (numArgs === 3) {
                ...
                that._drawCommands += ("d" + numArgs + "," + image + "," +
                    0 + "," + 0 + "," + imageInfo.width + "," + imageInfo.width + "," +
                    x + "," + y + "," + imageInfo.width + "," + imageInfo.width + ";");
            } else if (numArgs === 5) {
                var x = parseFloat(params.sx) || 0.0;
                var y = parseFloat(params.sy) || 0.0;
                var width = parseInt(params.sw) || imageInfo.width;
