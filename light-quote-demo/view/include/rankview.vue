<template>
	<div class="root">
	    ...
	    border-bottom-color: #f0f0f0;background-color:#F9F4EA" v-for="item in bottomlist" @click="onitemclick(item)">
	    ...
	    border-bottom-color: #f0f0f0;background-color:#F9F4EA;align-items:center;" v-for="item in bottomlist" @click="onitemclick(item)">
	    						<div style="flex-direction: column;" v-for="items in item.data">
									<text class="righttext"  :value="items.value" :style="{color:items.color,height:items.twolines?60:30,paddingTop:items.twolines?30:0}"></text>
		...
		methods:{
			...
			onitemclick:function(item){
				...
				if(stockDetailUserJsHead){
					...
					event.openURL(this.baseUrl + "app.native.js?stockCode=" + stockCode + "&codeType=" + codetype + "&stockName=" + encodeURIComponent(name) + "&hideCutoverImage=" + true + "#/navigator",{headHidden:true});
				}
				else{
					event.openURL(this.baseUrl + "app.native.js?stockCode=" + stockCode + "&codeType=" + codetype + "&stockName=" + encodeURIComponent(name) + "#/stockDetail");
				}
			},
