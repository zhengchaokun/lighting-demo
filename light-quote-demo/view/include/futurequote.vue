...
<template>
	<list style="background-color:#F9F4EA;">
		...
		methods: {
			...
			indexonclick: function(index) {
				...
				if(stockDetailUserJsHead){
	        			event.openURL(this.baseUrl + "app.native.js?stockCode=" + this.stockCodes[index] + "&codeType=" + this.codeTypes[index] + "&stockName=" + encodeURIComponent(this.stockNames[index]) + "#/navigator",{headHidden:true});
	        	}
	        	else{
					event.openURL(this.baseUrl + "app.native.js?stockCode=" + this.stockCodes[index] + "&codeType=" + this.codeTypes[index] + "&stockName=" + encodeURIComponent(this.stockNames[index]) + "#/stockDetail");
				}
			},
