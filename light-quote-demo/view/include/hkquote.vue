<template>
	<list>
						...
						<div style="height:164px;width:200px">
							<div style="flex-direction:column;">
								<div style="flex-direction:row;justify-content:center;">
		        					<image v-if="item.src!=''" style="margin-top:20px;width:88;height:66;" :src="item.src"></image>
								</div>
		        				<text v-if="item.src!=''"style="flex:1;font-size:28px;color:#333333;text-align:center;margin-top:10px;">{{item.prod_name}}</text>
		        			</div>
		        			<text v-if="item.src.length==0" style="flex:1;margin-top:20px;font-size:32px;color:#333333;margin-left: 16;">{{item.prod_name}}</text>

		        			<text v-if="item.src.length==0" style="flex:1;font-size: 34;font-weight: 600;margin-top:-10px;margin-left: 16;" :class="[getColorWithPriceChange(item.px_change)]">{{item.last_px}}</text>

			          		<div v-if="item.src.length==0" style="flex-direction:row;flex:1;margin-left: 16;">
				          		<text :class="[getColorWithPriceChange(item.px_change)]" style="margin-right:5px;font-size: 20;">{{getPriceChange(item.px_change)}}</text>

				          		<text :class="[getColorWithPriceChange(item.px_change)]" style="margin-left:5px;font-size: 20;">{{getPriceChangePercent(item.px_change_rate,item.px_change)}}</text>
			          		</div>
						</div>
	        		</div>
	      	</div>
		...
		<!-- 热门板块 -->
		<collapseview title="港股热门板块" rightTitle=" 更多 "  @rightonclick="onRightClick(ranklistparams[0])">
			<div class="hq_stocks">
			<div v-for="item in hotblock" class="item">
				<div style="align-items: center; width:250;" @click="onHotItem(item)">
          			<text class="prod_name" :style="{fontSize:item.prodNameFontSize}">{{item.prod_name}}</text>
    				<text style="flex:1;font-size: 36;font-weight: 600;margin-top:10px;" :class="[getColorWithPriceChange(item.px_change_rate)]">{{getPriceChangePercent(item.px_change_rate,0)}}</text>
    				<text style="font-size: 24;flex:1;color:#666666;margin-top:10px;">{{item.stock_name}}</text>
		...
		<!--涨幅榜 -->
		<collapseview title="主板涨幅榜" rightTitle=" ••• " rightFontColor="#ccc" @rightonclick="onRightClick(ranklistparams[1])">
		...
		methods:{
          ...
          //         },
	        ...
	        getHotBlockData:function(callback){
				...
				return this.getRankingStocksData({'en_hq_type_code':'XBHK.HSHY','sort_field_name':'px_change_rate','fields':'prod_name,hq_type_code,px_change_rate,rise_first_grp','data_count':3,'sort_type':'1'},callback);
	        },
	        getRiseRankList:function(callback){
	        	return this.getRankingStocksData({'en_hq_type_code':'XHKG-M.es','sort_field_name':'px_change_rate','fields':'prod_name,hq_type_code,last_px,px_change_rate,px_change,trade_status,special_marker','data_count':10,'sort_type':'1'},callback);

	        },
	        getFallRankList:function(callback){
				return this.getRankingStocksData({'en_hq_type_code':'XHKG-M.es','sort_field_name':'px_change_rate','fields':'prod_name,hq_type_code,last_px,px_change_rate,px_change,trade_status,special_marker','data_count':10,'sort_type':'0'},callback);
	        },
	        getTurnoverRatioList:function(callback){
				return this.getRankingStocksData({'en_hq_type_code':'XHKG-M.es','sort_field_name':'turnover_ratio','fields':'prod_name,hq_type_code,last_px,turnover_ratio,trade_status,special_marker','data_count':10,'sort_type':'1'},callback);
	        },
          ...
          //         },
	        ...
	        onIndexItem: function(item){
	        	if(item.gmu){
					if(item.pageId == "hgtpage"){
						event.openURL(this.baseUrl + "app.native.js?type=HLHGTBalance#/hkSpecialSubject");
					}
					else{
						event.openURL(this.baseUrl + "app.native.js?type=HLSGTBalance#/hkSpecialSubject");
					}
	        	}else{
				...
				this.getRealtimeList(e =>{
		            ...
		            for (var key in result){
		                if ("fields" != key) {
		                    ...
		                    var p = {};
		                    p.last_px = parseFloat(result[key][fields.indexOf("last_px")]).toFixed(3);
		                    p.px_change = parseFloat(result[key][fields.indexOf("px_change")]).toFixed(3);
		            ...
		            var arr =that.hkstocks.split(",");
		            if(that.cnIndexList.length*3<arr.length+2){
						//globalquote#hgtpage
		            	newArr.push({"prod_name":"沪港通专题","buttonTitle":"沪港通专题", "gmu":"globalquote","pageId":"hgtpage", src: that.baseUrl + "image/shhk.png"});
		            	newArr.push({"prod_name":"深港通专题","buttonTitle":"深港通专题", "gmu":"globalquote","pageId":"sgtpage", src: that.baseUrl + "image/szhk.png"});
		            	that.cnIndexList.push(newArr);
		            	newArr=[];
		            }
				});
				...
				this.getHotBlockData(e =>{
			            ...
			            for (var r in a) {
			                ...
			                if ("fields" != r) {
			                    ...
			                    n.stock_name = l[1];
			                    n.stock_price = parseFloat(l[2]).toFixed(3);
				...
				this.getRiseRankList(e =>{
		            ...
		            for (var r in a) {
		                ...
		                if ("fields" != r) {
		                    ...
		                    n.prod_code =r.split('.')[0];
		                    n.prod_code_all=r;
		                    n.last_px = parseFloat(a[r][s.indexOf("last_px")]).toFixed(3);
		                    n.px_change_rate =parseFloat(a[r][s.indexOf("px_change_rate")]).toFixed(2);
							...
							n.marketicon =getMarketIcon(n.hq_type_code);
