		   	...
		   	<div v-for="(v,i) in cnIndexList" class="cnIndex">
	        		<div v-for="(item,k) in v" class="indexItem" @click="onIndexItem(item)">
						<div style="height:164px;width:200px">
		        			...
		        			<text v-if="item.src.length==0" style="flex:1;margin-top:20px;font-size:32px;color:#333333;margin-left: 22;">{{item.prod_name}}</text>
...
<script>
     ...
     //              },
          ...
          //         },
	        ...
	        onIndexItem: function(item){
				if (item.marketslist) {
					event.openURL(this.baseUrl + "app.native.js?markets_params=" + encodeURIComponent(JSON.stringify(item)) + "&fields=" + this.stock_fields + "#/stockrank");
				} else {
	...
	　　　　  return true;
    	...
    	created:function(){
			...
			storage.getItem('isFirstStart', event => {
                  var result=event.data;
                  if(result=="undefined"){
                     quoteDc.saveMyStocks("XSHG-601988,XSHG-601288,XSHG-601398,XSHG-601939,XSHG-601328", null);
