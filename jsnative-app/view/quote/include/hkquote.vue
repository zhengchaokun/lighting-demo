		   	...
		   	<div v-for="(v,i) in cnIndexList" class="cnIndex">
	        		<div v-for="(item,k) in v" class="indexItem" @click="onIndexItem(item)">
						<div style="height:164px;width:200px">
		        			...
		        			<text v-if="item.src.length==0" style="flex:1;margin-top:20px;font-size:28px;color:#333333;margin-left: 16;">{{item.prod_name}}</text>
	        ...
	        onIndexItem: function(item){
	        	if(item.gmu){
					if(item.pageId == "hgtpage"){
						event.openURL(this.baseUrl + "hkSpecialSubject.js?type=HLHGTBalance");
					}
					else{
						event.openURL(this.baseUrl + "hkSpecialSubject.js?type=HLSGTBalance");
					}
	        	}else{
