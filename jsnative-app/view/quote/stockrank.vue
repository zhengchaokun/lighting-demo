<template>
	<div @viewappear="onViewappear" @viewdisappear="onViewdisappear">
		...
		<div v-if="headinfoshow" @click="headInfoClick(stock)" style="flex-direction: row;justify-content: space-between;align-items: center;padding: 20;height: 110;">
			<div>
		...
		methods: {
	        ...
	        headInfoClick: function(stock) {
	        	if(stockDetailUserJsHead){
	        			event.openURL(this.baseUrl + "navigator.js?stockCode=" + stock.stockCode + "&codeType=" + stock.codeType + "&stockName=" + encodeURIComponent(stock.stockName),{headHidden:true});
	        	}
	        	else{
	        		event.openURL(this.baseUrl + "stockDetail.js?stockCode=" + stock.stockCode + "&codeType=" + stock.codeType + "&stockName=" + encodeURIComponent(stock.stockName));
	        	}
	        },
			...
			if (!this.isEmptyObject(this.markets_params)) {
				...
				if (this.markets_params.marketslist) {
					...
					if (this.markets_params.type == "block_rank") {
						this.tabBarBackgroundColor = "#EAEAEA";
					}
