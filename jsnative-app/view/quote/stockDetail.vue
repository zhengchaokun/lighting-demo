<template>
    <list style="flex-direction:column;" focustype="block" @viewappear="onViewappear" @viewdisappear="onViewdisappear" :scrollDisabled="isScrollDisabled">
          ...
          <div style="flex:1">
            ...
            <div>
              <div class="realtimeRight">
                <text style="color:#9C9C9C;font-size: 22;margin-top:20;text-align: left;flex:1;" :value="realTimeRightText11"></text>
                <text style="color:#9C9C9C;font-size: 22;margin-top:20;text-align: left;flex:1;margin-left:10;" :value="realTimeRightText12"></text>
                <text style="color:#9C9C9C;font-size: 22;margin-top:20;text-align: left;flex:1;margin-left:10;" :value="realTimeRightText13"></text>
              </div>
              <div class="realtimeRight">
                <text style="color:#000000;font-size: 22; text-align: left;flex:1;" :value="realTimeRightText21"></text>
                <text style="color:#000000;font-size: 22;text-align: left;flex:1;margin-left:10;" :value="realTimeRightText22"></text>
                <text style="color:#000000;font-size: 22;text-align: left;flex:1;margin-left:10;" :value="realTimeRightText23"></text>
              </div>
            </div>
            <!-- 第二排-->
            <div>
              <div class="realtimeRight">
                <text style="color:#9C9C9C;font-size: 22;margin-top:20;text-align: left;flex:1;" :value="realTimeRightText31"></text>
                <text style="color:#9C9C9C;font-size: 22;margin-top:20;text-align: left;flex:1;margin-left:10;" :value="realTimeRightText32"></text>
                <text style="color:#9C9C9C;font-size: 22;margin-top:20;text-align: left;flex:1;margin-left:10;" :value="realTimeRightText33"></text>
              </div>
              <div class="realtimeRight">
                <text style="color:#000000;font-size: 22;text-align: left;flex:1;" :value="realTimeRightText41"></text>
                <text style="color:#000000;font-size: 22;text-align: left;flex:1;margin-left:10;" :value="realTimeRightText42"></text>
                <text style="color:#000000;font-size: 22;text-align: left;flex:1;margin-left:10;" :value="realTimeRightText43"></text>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style="flex-direction: column;border-bottom-width: 0.5;border-bottom-color:#B5B5B5;" v-if="isShowRealTimeBottom">
        <!--第一排-->
        ...
        <div style="height:20;background-color:#f3f3f3"></div>


        <div style="height:61" :class="[foucusMarginStyle(isFocusMargin)]">
          
          <!-- 分时，5日等导航栏 -->
          <jstabbar ref="jstabbar" style="position:absolute;left:0;top:0;" :tabItems="chartTabItems" :indexOfExtra="5" :iconurl="isShowExtra?(baseUrl+'image/tabbar_up.png'):(baseUrl+'image/tabbar_down.png')" @tabBarOnClick="switchType" titleColor="#666"  :style="{ visibility: !isShowFocusInfo ? 'visible':'hidden',height:!isShowFocusInfo ?61:0} "></jstabbar>
        ...
        <div class="upfalllist" v-if="isUpDownListShow" >
            <jstabbar :tabItems="UpDownTabItems" selectedColor="#f75e61" @tabBarOnClick="switchUpOrFall" style="height:70"></jstabbar>
        ...
        <div :style="{height:extandDivHeight}" v-if="isShowExtandDiv">
        </div>
        </cell>
    </list>
</template>
        ...
        getSnapshotdata:function(){
            ...
            for(bid;bid<that.snapshotdata.data.snapshot.fields.length;bid++){
              if(that.snapshotdata.data.snapshot.fields[bid] == 'bid_grp'){
                  ...
                  if(that.hasData(open) && parseFloat(open)!=0){
                      open = that.formatBigNumber(open,that.bit);
                  }else{
                      open = '--';
                  }
                  // log.i("开盘价"+open);
                  ...
                  if(that.hasData(high) && parseFloat(high)!=0){
                      high = that.formatBigNumber(high,that.bit);
                  }
                  else
                  {
                      high = '--';
                  ...
                  if(that.hasData(low) && parseFloat(low)!=0){
                      low = that.formatBigNumber(low,that.bit);
                  }
                  else
                  {
                      low = '--';
                  ...
                  if(that.hasData(iopv) && parseFloat(iopv)!=0){
                      iopv = that.formatBigNumber(iopv/100,that.bit);
                  }
                  else
                  {
                      iopv = '--';
                  ...
                  if(that.hasData(amountdelta) && parseFloat(amountdelta)!=0){
                      if(Math.abs(Number(amountdelta))>=100000){
                          amountdelta = that.formatBigNumber(amountdelta,2);
                      }
                      else
                      {
                          amountdelta = Number(amountdelta).toFixed(0);
                      }
                  }
                  else
                  {
                ...
                // log.i("日增"+amountdelta);
                  ...
                  if(that.hasData(amount) && parseFloat(amount)!=0){
                  ...
                  if(that.hasData(totalmaney) && parseFloat(totalmaney)!=0){
              ...
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'vol_ratio'){
                  volumeratio = that.snapshotdata.data.snapshot[stockCode][bid];
                  if(that.hasData(volumeratio) && parseFloat(volumeratio) > 0){
                      volumeratio = that.formatBigNumber(volumeratio,2);
                  }else{
                      volumeratio = '--';
                  }
                  // log.i("量比"+volumeratio);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'prev_settlement'){
                prevsettlement = that.snapshotdata.data.snapshot[stockCode][bid];
                // log.i("昨结"+prevsettlement);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'business_amount_in'){
                  amountIn = that.snapshotdata.data.snapshot[stockCode][bid];
                  if(parseFloat(amountIn)==0){
                    amountIn = '--';
                  }
                   //log.i("内盘"+amountIn);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'business_amount_out'){
                  amountOut = that.snapshotdata.data.snapshot[stockCode][bid];
                  if(parseFloat(amountOut)==0){
                    amountOut = '--';
                  }
                   //log.i("外盘"+amountOut);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'pe_rate'){
                  peRate = that.snapshotdata.data.snapshot[stockCode][bid];
                  // log.i("市盈率"+peRate);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'circulation_value'){
                  circulationValue = that.snapshotdata.data.snapshot[stockCode][bid];
                  // log.i("流通市值"+circulationValue);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'circulation_amount'){
                  circulationAmount = that.snapshotdata.data.snapshot[stockCode][bid];
                  // log.i("流通股本"+circulation_amount);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'total_shares'){
                  totalShares = that.snapshotdata.data.snapshot[stockCode][bid];
                  // log.i("总股本"+total_shares);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'up_px'){
                  up = that.snapshotdata.data.snapshot[stockCode][bid];
                  if(that.hasData(up)){
                  ...
                  // log.i("涨停"+up);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'down_px'){
                  down = that.snapshotdata.data.snapshot[stockCode][bid];
                  ...
                  // log.i("跌停"+down);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'dyn_pb_rate'){
                  dynPbRate = that.snapshotdata.data.snapshot[stockCode][bid];
                  // log.i("市净率"+dynPbRate);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'w52_high_px'){
                  high52 = that.snapshotdata.data.snapshot[stockCode][bid];
                  if(that.hasData(high52) && parseFloat(high52)!=0){
                      high52 = that.formatBigNumber(high52,that.bit);
                  }else{
                      high52 = '--';
                  }
                  // log.i("52周最高"+high52);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'w52_low_px'){
                  low52 = that.snapshotdata.data.snapshot[stockCode][bid];
                  if(that.hasData(low52) && parseFloat(low52)!=0){
                      low52 = that.formatBigNumber(low52,that.bit);
                  }else{
                      low52 = '--';
                  }
                  // log.i("52周最低"+low52);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'market_value'){
                  marketValue= that.snapshotdata.data.snapshot[stockCode][bid];
                  // log.i("总市值"+marketValue);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'amplitude'){
                  amplitude= that.snapshotdata.data.snapshot[stockCode][bid];
                  ...
                  //log.i("振幅"+amplitude);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'avg_px'){
                  avg= that.snapshotdata.data.snapshot[stockCode][bid];
                  if(that.hasData(avg)){
                  ...
                  if(that.hasData(risecount) && parseFloat(risecount)!=0){
                      risecount = that.formatBigNumber(risecount,0);
                  }else{
                      risecount = '--';
                  }
                  // log.i("涨家数"+risecount);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'fall_count'){
                  fallcount= that.snapshotdata.data.snapshot[stockCode][bid];
                  if(that.hasData(fallcount) && parseFloat(fallcount)!=0){
                      fallcount = that.formatBigNumber(fallcount,0);
                  }else{
                      fallcount = '--';
                  }
                  // log.i("跌家数"+fallcount);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'member_count'){
                  totalcCount= that.snapshotdata.data.snapshot[stockCode][bid];
                  if(that.hasData(totalcCount) && parseFloat(totalcCount)!=0){
                      totalcCount = that.formatBigNumber(totalcCount,0);
                  }else{
                      totalcCount = '--';
                  }
                  // log.i("总家数"+totalcCount);
              }else if(that.snapshotdata.data.snapshot.fields[bid] == 'popc_px'){
                  popcPrice= that.snapshotdata.data.snapshot[stockCode][bid];
                  if(parseFloat(popcPrice)==0){
                    popcPrice = '--';
                  }
                  // log.i("盘前后价"+popcPrice);
      ...
      setWebviewHeight: function(id){
        ...
        hide:function(id){
          ...
          changeColor:function(id){
              ...
              switch(id){
                  case 'news':
                  this.newscolor = "#f75e61";
                  this.neborder = "#f75e61";
                  break;
                  case 'fund':
                  this.fundcolor = "#f75e61";
                  this.fuborder = "#f75e61";
                  break;
                  case 'notice':
                  this.noticecolor = "#f75e61";
                  this.noborder = "#f75e61";
                  break;
                  case 'study_report':
                  this.study_reportcolor = "#f75e61";
                  this.sborder = "#f75e61";
                  break;
                  case 'f10':
                  this.f10color = "#f75e61";
                  this.f10border = "#f75e61";
                  break;
                  case 'information':
                  this.informationcolor = "#f75e61";
                  this.inborder = "#f75e61";
                  break;
              }
          },
          ...
          setInfoItems: function(isFirst){
            ...
            if(productType == "PRODUCT_FUND") {
                ...
                self.noticeUrl = 'https://light.hscloud.cn/apps/minfo/jjgonggao.html?s=' + self.stockCode + '&p=HSJY_1025&u=hundsun&tg=_blank&n=1&h=0';
                self.f10Url =  'https://light.hscloud.cn/apps/minfo/jjf10.html?s=' + self.stockCode + '&p=HSJY_1025&u=hundsun&tg=_blank&n=1&h=0';
                if(isFirst){
                ...
                //沪深股票
                //log.i("沪深股票");
                self.newsUrl = 'https://light.hscloud.cn/apps/v3_xin_wen_zi_xun/index.html?stock_code=' + self.stockCode + (ss.length>0?("."+ss):'');
                self.fundUrl = 'https://rqfcehxot.lightyy.com/index.html#/fundflow?stock_code=' + self.stockCode + (ss.length>0?("."+ss):'');
                self.noticeUrl = 'https://light.hscloud.cn/apps/v3_xin_wen_zi_xun/index.html?stock_code=' + self.stockCode + (ss.length>0?("."+ss):'') + '&info_type=notice';
                self.studyReportUrl = 'https://light.hscloud.cn/apps/v3_xin_wen_zi_xun/index.html?stock_code=' + self.stockCode + (ss.length>0?("."+ss):'') + '&info_type=report';
                self.f10Url =  'https://light.hscloud.cn/apps/minfo/f10.html?s=' + self.stockCode + '&p=HSJY_1025&u=hundsun&tg=_blank&n=1&h=0';
                ...
                self.newsUrl = 'https://www.lightyy.com/apps/v3_xin_wen_zi_xun-test/index.html?stock_code=' + self.stockCode + (ss.length>0?("."+ss):'')+'&secu_market=us';
                self.fundUrl = 'https://www.lightyy.com/apps/v3_xin_wen_zi_xun-test/info.html?stock_code=' + self.stockCode + (ss.length>0?("."+ss):'') +'&info_type=1';
                self.f10Url =  'https://www.lightyy.com/apps/v3_xin_wen_zi_xun-test/info.html?stock_code=' + self.stockCode+(ss.length>0?("."+ss):'') +'&info_type=0';
                //美股
                //log.i("美股");
                if (!self.isTabviewShow){
                  self.isTabviewShow = true;
                ...
                //港股
                //log.i("港股");
                self.newsUrl = 'https://light.hscloud.cn/apps/minfo/hknews.html?s='+ self.stockCode +'&p=HSJY_1025&u=hundsun&tg=_blank&n=1&h=0';
                self.noticeUrl = 'https://light.hscloud.cn/apps/minfo/hkgonggao.html?s='+ self.stockCode +'&p=HSJY_1025&u=hundsun&tg=_blank&n=1&h=0';
                self.studyReportUrl = 'https://light.hscloud.cn/apps/minfo/hkyanbao.html?s='+ self.stockCode +'&p=HSJY_1025&u=hundsun&tg=_blank&n=1&h=0';
                self.f10Url =  'https://light.hscloud.cn/apps/minfo/hkf10.html?s='+ self.stockCode +'&p=HSJY_1025&u=hundsun&tg=_blank&n=1&h=0';
                if (!self.isTabviewShow){
                  self.isTabviewShow = true;
                }
                if(isFirst){
              ...
              //self.isShowRealTimeBottom = false;
                ...
                //期货
                //log.i("期货");
                self.informationUrl = 'https://www.lightyy.com/apps/v3_xin_wen_zi_xun-test/info.html?stock_code='+ self.stockCode + (ss.length>0?("."+ss):'') +'&info_type=2';
                if (!self.isTabviewShow){
                  self.isTabviewShow = true;
                }
                if(isFirst){
        ...
        goLandscape:function(){
          ...
          event.openURL(this.baseUrl + "landscapeStockDetail.js?stockCode=" + this.stockCode + "&codeType=" + this.codeType + "&stockName=" + encodeURIComponent(this.stockName), {landscape:true,headHidden:true});
        },
